...
- 首页（logo+导航、广告位、最新动态列表 + "更多"链接跳转 `/news.html`、专家信息滚动展示 + "更多"链接跳转 `/experts.html`、矿权融资预览 + "更多"链接跳转 `/finance.html`、中部广告位）
  ...
  - **新增分块广告位**：支持 `home_tiles` 分块广告展示（可在后台广告管理维护）
- 关于我们
...
**个人信息与密码维护**：
- 用户中心支持直接维护个人信息（姓名、职务、邮箱、手机、单位、业务范围）
- 用户中心支持修改个人密码

**在线洽谈登录保护**：
- 已登录用户在 `/finance-detail.html` 可直接留言，不再重复填写姓名与电话（后端自动取账号实名与手机号）。
...
| 用户管理   | 密码管理              | 管理员修改自己密码、按用户名重置用户密码      |
...
- `PUT /api/admin/password` — 管理员修改自身密码
- `PUT /api/admin/end-users/reset-password` — 按用户名重置用户密码
- `POST /api/admin/upload-image` — 后台图片上传（用于广告/专家等）
